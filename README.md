...
- To login with the pretended profile go to the login page, located at: [http://localhost:8000/login](http://localhost:8000/login);
...
- To access the protected resources you must add the JWT token to the `Authorization` header of the request with the following format:
  | key | value |
  | --------------- | --------------------- |
